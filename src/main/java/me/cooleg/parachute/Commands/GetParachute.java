...
public class GetParachute implements CommandExecutor {

    private final ItemStack parachute;
    private final boolean stack;
    private final NamespacedKey random;

    public GetParachute(Parachute main, ConfigWrapper config) {
        parachute = new ItemStack(config.getParachuteMat());
        stack = config.isStack();
        random = new NamespacedKey(main, "unstack");
        ItemMeta meta = parachute.getItemMeta();
        meta.setDisplayName(config.getRegularName());
        meta.setLore(Collections.singletonList(config.getRegularDescription()));
        meta.setCustomModelData(config.getItemData());
        meta.getPersistentDataContainer().set(new NamespacedKey(main, "item"), PersistentDataType.STRING, "parachute");
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
        ...
        if (args.length == 0) {
            ...
            commandSender.sendMessage(ChatColor.GREEN + "You were given a parachute!");
            return true;
            ...
            commandSender.sendMessage(ChatColor.GREEN + args[0] + " was given a parachute!");
            return true;
        } else {
            Player p = Bukkit.getPlayer(args[0]);
            if (p == null) {commandSender.sendMessage(ChatColor.RED + "This player doesn't exist!"); return true;}
            ...
            commandSender.sendMessage(ChatColor.GREEN + args[0] + " was given parachutes!");
            return true;
        }
    }
}
